...
export class QuestionService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/questions");
  ...
  update(question: IQuestion): Observable<EntityResponseType> {
    ...
    return this.http
      .put<IQuestion>(
        `${this.resourceUrl}/${getQuestionIdentifier(question) as number}`,
        copy,
        { observe: "response" }
      )
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http
      .get<IQuestion[]>(this.resourceUrl, {
        params: options,
        observe: "response",
      })
      .pipe(
  ...
  addQuestionToCollectionIfMissing(
    questionCollection: IQuestion[],
    ...questionsToCheck: (IQuestion | null | undefined)[]
  ): IQuestion[] {
    const questions: IQuestion[] = questionsToCheck.filter(isPresent);
    if (questions.length > 0) {
      const questionCollectionIdentifiers = questionCollection.map(
        (questionItem) => getQuestionIdentifier(questionItem)!
      );
      const questionsToAdd = questions.filter((questionItem) => {
        ...
        if (
          ...
          questionCollectionIdentifiers.includes(questionIdentifier)
        ) {
          return false;
        }
  ...
  protected convertDateFromClient(question: IQuestion): IQuestion {
    return Object.assign({}, question, {
      createdDate: question.createdDate?.isValid()
        ? question.createdDate.toJSON()
        : undefined,
      lastModifiedDate: question.lastModifiedDate?.isValid()
        ? question.lastModifiedDate.toJSON()
        : undefined,
      archivedDate: question.archivedDate?.isValid()
        ? question.archivedDate.toJSON()
        : undefined,
    });
  }
    ...
    if (res.body) {
      res.body.forEach((question: IQuestion) => {
        question.createdDate = question.createdDate
          ? dayjs(question.createdDate)
          : undefined;
        question.lastModifiedDate = question.lastModifiedDate
          ? dayjs(question.lastModifiedDate)
          : undefined;
        question.archivedDate = question.archivedDate
          ? dayjs(question.archivedDate)
          : undefined;
      });
    }
    return res;
